...
protected:
  void registerOptionsAndFlags_()
  {
    ...
    registerDoubleOption_("intensity_tolerance", "<tolerance>", 0.00001, "Intensities below this value will be set to 0.0 to avoid numerical problems when quantifying.", false);
  ...
  */
  ...
  ExitCodes main_(int, const char **)
  {
    ...
    intensity_tolerance_ = getDoubleOption_("intensity_tolerance");

    // check if output file is valid
    String tsvFileName = getStringOption_("out");
    TextFile textFile;

    textFile.push_back(ListUtils::concatenate(construct_header(), "\t"));
        ...
        ++cm_iter)
    {
           ...
           ++entry)
      {
        ...
        for(ConsensusFeature::HandleSetType::const_iterator fIt = features.begin();
            ...
            ++fIt)
        {
          intensityMap[Int(fIt->getMZ())] = (fIt->getIntensity() > intensity_tolerance_ ? fIt->getIntensity() : 0.0 );
        }
        for(IsobaricQuantitationMethod::IsobaricChannelList::const_iterator it = selected_quant_method_->getChannelInformation().begin();
            it != selected_quant_method_->getChannelInformation().end();
            ++it)
